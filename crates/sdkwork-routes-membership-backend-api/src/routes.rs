...
pub fn build_membership_backend_router(host: Arc<MembershipServiceHost>) -> Router {
    match host.database_pool() {
        DatabasePool::Postgres(pool, _) => admin_membership_router_with_postgres_pool(pool.clone()),
        DatabasePool::Sqlite(pool, _) => admin_membership_router_with_sqlite_pool(pool.clone()),
    }
}
