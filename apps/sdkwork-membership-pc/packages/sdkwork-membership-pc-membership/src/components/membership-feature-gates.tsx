...
import {
  SDKWORK_MEMBERSHIP_FEATURE_CODES,
  type SdkworkMembershipFeatureGate,
 ...
 * 展示各功能的所需等级与锁定/解锁状态。
 */
export function SdkworkMembershipFeatureGates({
  service,
}: SdkworkMembershipFeatureGatesProps) {
  const { copy } = useSdkworkMembershipIntl();
  const [gates, setGates] = useState<SdkworkMembershipFeatureGate[] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    const features = SDKWORK_MEMBERSHIP_FEATURE_CODES as readonly string[];
  ...
  return (
    <section
      ...
      data-sdkwork-membership-feature-gates
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs leading-5 text-[var(--sdk-color-text-muted)]">
              {copy.gates.description}
            </p>
          </div>
        </div>

        {!gates ? (
          <p className="mt-4 text-xs text-[var(--sdk-color-text-muted)]">
            {copy.page.loading}
          </p>
        ) : (
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {items.map((gate) => {
              ...
              return (
                <li
                  className="flex items-center justify-between gap-3 rounded-xl border border-[var(--sdk-color-border-default)] bg-[var(--sdk-color-surface-raised)] px-3 py-2.5"
                  key={gate.featureCode}
                >
                  <span className="flex min-w-0 items-center gap-2 text-sm text-[var(--sdk-color-text-strong)]">
                    <Icon
                      className={`h-4 w-4 shrink-0 ${unlocked ? "text-[var(--sdk-color-brand)]" : "text-[var(--sdk-color-text-muted)]"}`}
                      aria-hidden="true"
                    />
                  ...
                  <span className="flex shrink-0 items-center gap-2">
                    <span className="text-xs text-[var(--sdk-color-text-muted)]">
                      {copy.gates.requiredLevel} {gate.requiredLevel}
                    </span>
                    <span
                      className="rounded-full px-2 py-0.5 text-xs font-semibold"
                      style={unlocked
                        ? createSdkworkMembershipToneStyle("success")
                        : createSdkworkMembershipToneStyle("warning")}
                    >
                      {unlocked ? copy.gates.unlocked : copy.gates.locked}
                    </span>
                  </span>
                </li>
              );
